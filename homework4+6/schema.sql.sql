-- 切换到目标数据库（确保数据库已创建：CREATE DATABASE IF NOT EXISTS course_select;）
USE course_select;
...
-- 5. 选课记录表（enrollment）：保留外键和唯一约束（无兼容性问题）
